...
public class CreateWorkspaceRestAction extends BaseRestHandler {
    @Override public String getName() { return "create_workspace"; }
    @Override public List<Route> routes() { return List.of(new Route(RestRequest.Method.POST, Constants.WORKSPACES_API_BASE)); }
    @Override @SuppressWarnings("unchecked")
    protected RestChannelConsumer prepareRequest(RestRequest request, NodeClient client) throws IOException {
        Map<String, Object> body = request.contentParser().map();
        CreateWorkspaceRequest actionRequest = new CreateWorkspaceRequest(
            (String) body.get("name"),
            (String) body.getOrDefault("description", ""),
            (List<String>) body.getOrDefault("features", List.of()),
            (String) body.getOrDefault("color", "")
        );
        return channel -> client.executeLocally(CreateWorkspaceAction.INSTANCE, actionRequest, new RestToXContentListener<>(channel));
    }
}
