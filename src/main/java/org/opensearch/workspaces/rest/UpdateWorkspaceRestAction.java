...
public class UpdateWorkspaceRestAction extends BaseRestHandler {
    @Override public String getName() { return "update_workspace"; }
    @Override public List<Route> routes() { return List.of(new Route(RestRequest.Method.PUT, Constants.WORKSPACES_API_BASE + "/{id}")); }
    @Override @SuppressWarnings("unchecked")
    protected RestChannelConsumer prepareRequest(RestRequest request, NodeClient client) throws IOException {
        Map<String, Object> body = request.contentParser().map();
        UpdateWorkspaceRequest actionRequest = new UpdateWorkspaceRequest(
            request.param("id"),
        ...
        return channel -> client.executeLocally(UpdateWorkspaceAction.INSTANCE, actionRequest, new RestToXContentListener<>(channel));
    }
}
